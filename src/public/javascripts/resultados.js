document.addEventListener('DOMContentLoaded', () => {
  ...
  fetch(`/api/respuestas/${idEncuesta}`, {
    headers: {
    ...
    .then(data => {
        ...
        // Obtener nombre y descripción de la encuesta
        const { nombre, descripcion, respuestas } = data;

        // Actualizar el encabezado con el nombre y descripción
        ...
        respuestas.forEach(item => {
          ...
          if (item.tipo === "opcion-multiple") {
              ...
              const totalRespuestas = item.respuestas.length;
              ...
              <ul class="list-unstyled">
                  ${Object.entries(conteoOpciones)
                      .map(([opcion, conteo]) => {
                          const porcentaje = ((conteo / totalRespuestas) * 100).toFixed(1); // Porcentaje
                          return `
                              <li class="mb-3">
